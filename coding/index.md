...
- `PS-GEN-RULE-XXX` generic rules for all supported languages (C and assembly), which are formulated using `shall` what
indicates mandatory action;
- `PS-GEN-REC-XXX` generic recommendations  for all supported languages (C and assembly),
which are formulated using `should`;
...
This document does not put any constraints on coding tool used by the developer.
...
Source files should have the following file extensions:
...
[MISRA official git repository](https://gitlab.com/MISRA/MISRA-C/MISRA-C-2012/tools/-/blob/main/misra_c_2023__headlines_for_cppcheck.txt?ref_type=heads)
...
Global variables should only be placed in structures.

##### Note

Structures can be used to group all global variables related to a specific module or subsystem. The goal is to organize
global state in one place, making the code easier to maintain and reducing the risk of naming conflicts.
...
The header with the `#include` preprocessing directive should be placed after the label and
