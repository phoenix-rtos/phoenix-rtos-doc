...
It consit of sections, which are grouping coding rules from one topic.
Coding rules are identified with ID of such format: `PS-CODE-XXX`.
Rules are formulated with use of these speciall statements:
...
### PS-CODE-005
Main label blocks shall be separated with empty line. 
...
- present copyright notices the file with newest copyrights at the top and authors sorted according to the importance of their contribution
...
## Indentation
### PS-CODE-007
The tabulator shall be used as code indentation.
...
### PS-CODE-014
The subsystem name shall be a one word without the underline characters. The functionality
...
### Example
Lets say we have library `libfoo` and it's function called `init`. This function must be prefixed with
...
## Variables
### PS-CODE-021
Variables should be named with one short word without the underline characters. 

### PS-CODE-022
For variable nams longer than one word, the camelCase shall be used. 
...
### PS-CODE-026
The stack usage and number of local variables should beminimized in the kernel.
...
### PS-CODE-033
Global variables scope should be limited whenever possible by using `static`. 

### PS-CODE-034
Global variables can only be placed in common structures. 
...
```
### PS-CODE-036
Module name could be ommited in user space applications (i.e. not in the kernel) and name the structure
...
```
### PS-CODE-039
The `sizeof` and `typeof` shall be treated as functions and are to be used in accordance to the following notation:
...
```
### PS-CODE-040
If the increment `++` or decrement `--` operator are postfixed, no space shall be used before the operator. 

### PS-CODE-041
If the increment `++` or decrement `--` operatorare prefixed, no space should be used after the operator.
...
## Type definition
### PS-CODE-046
New types could only be defined if it is absolutely necessary. 
...
### PS-CODE-050
Comments should not contain coppied parts of specification

### Note:
Commentsare not the place to express programmer's novel writing skills.

### PS-CODE-051
Any kind of documentation generator (e.g. Doxygen) shall not be used.
...
#### Note: 
The use of preprocessing conditionals makesit harder to follow the code logic and can be used only if it is absolutely necessary.
...
### PS-CODE-058
Operating system messages shall consist of, in order:
- a subsystem name
- acolon
- a message body
- a newline character
