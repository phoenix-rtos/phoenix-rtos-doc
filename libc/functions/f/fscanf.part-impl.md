...
comprise the scanset, unless the byte after the `<left-square-bracket>` is a `<circumflex>` `( '^' )`, in which case
the scanset contains all bytes that do not appear in the scanlist between the `<circumflex>` and the `]`. If the
