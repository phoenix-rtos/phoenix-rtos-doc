...
shall support the freeing of arbitrary sublists of an `addrinfo` list originally returned by `getaddrinfo()`.
...
flag is specified along with an `ai_family` of `AF_INET6`, then `getaddrinfo()` shall return `IPv4`-mapped `IPv6`
...
If the `AI_ADDRCONFIG` flag is specified, `IPv4` addresses shall be returned only if an `IPv4` address is configured on
the local system, and `IPv6` addresses shall be returned only if an `IPv6` address is configured on the local system.
