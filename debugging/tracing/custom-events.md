...
The `tsdl/metadata` file is a CTF metadata stream conforming to
[CTF 1.8](https://diamon.org/ctf/v1.8.3/). It is a contract to the outside world
about the events understandable by the kernel. The kernel emits events in binary
form and the `tsdl/metadata` is the only source of information about the
structure of our events that the babeltrace2 has when parsing the binary streams
(this is what happens under the hood in `ctf_to_proto.py` which is under the
hood of `convert.sh`).

```{warning}
When adding custom events, you must ensure `trace-events.h` matches
`tsdl/metadata`, **otherwise the trace will be impossible to parse by the host
utilities.**
```
