...
* A way of transporting files from your target to host PC (likely
  platform-dependent)
...
This starts a trace for 3 seconds and captures it periodically (each 100 ms) to
`my_trace` directory. You can fine-tune the priority of the applet with `-p`,
change the sleeptime to alter periodical reads with `-s` or modify the capture
buffer size.
...
If you are gathering the trace on some QEMU target though, see
{doc}`obtaining-trace-from-qemu`.
...
```{shell-transcript}
[user] cd $PRTOS_PROJECT/phoenix-rtos-hostutils/trace
[user] ./convert.sh $TRACE_DIR $PRTOS_PROJECT/phoenix-rtos-kernel/perf/tsdl/metadata out.pftrace
```
