...
To share disk image to the bootloader, `phoenixd` has to be launched with the following arguments (choose suitable ttyACMx device, in this case, ttyACM0):
...
Alternatively Phoenix-RTOS can be run from RAM. In order to place the disk image on the board, `psu` (Phoenix Serial Uploader) and the `imx6ull-ram.sdp` (Serial Download Protocol) script should be used. 
