...
artifacts, including the system image, must be present in the `_boot` directory. If you have not built the system
...
```
grmon> load phoenix-rtos-project/_boot/sparcv8leon-gr716-mini/plo-ram.img 0x31000000
```

To verify that the file has been loaded correctly, run the following command:

```
grmon> verify phoenix-rtos-project/_boot/sparcv8leon-gr716-mini/plo-ram.img 0x31000000
```
...
The bootloader interface appears in the console.
...
During the system build, the `plo` console prints the address for the BCH error correction codes, as shown below:

```
BIN plo-sparcv8leon3-gr716.img
Converted <project-root>/_build/sparcv8leon3-gr716-mini/prog.stripped/plo-sparcv8leon3-gr716.elf
to <project-root>/_build/sparcv8leon3-gr716-mini/prog.stripped/plo-sparcv8leon3-gr716.img
Generated BCH of <project-root>/_build/sparcv8leon3-gr716-mini/prog.stripped/plo-sparcv8leon3-gr716.img
to <project-root>/_boot/sparcv8leon3-gr716-mini/plo.bch
Please load the BCH file to the SPI flash at offset 0xffcb20
```

In this case, write the file to `0xffcb20`. To do this, write the following command in the console with the `plo`
interface:

```
(plo)% copy uart3 plo.bch flash0 0xffcb20 0x0
```
