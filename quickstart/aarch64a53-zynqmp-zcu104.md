...
If there wasn't an older system image in the NOR flash the following output
should appear (here the first prompt, in green, has been generated by
Phoenix-RTOS, then the reset button has been used to reset the board,
in Your case, only the plo log and prompt, in purple, will be probably
visible):
...
On ZynqMP plo is configured with a RAM disk at address 0x08000000. The flash image
can be written to it using OpenOCD over JTAG which is a lot faster than over UART.

JTAG link configuration for OpenOCD is provided in `scripts/openocd/zynqmp/ftdi_zcu104.cfg`. If You prefer You can create `ftdi_zcu104.cfg` file in other location.
...
```sh
cd _boot/aarch64a53-zynqmp-zcu104
openocd -f "$(realpath ../../scripts/openocd/zynqmp/ftdi_zcu104.cfg)" -f "target/xilinx_zynqmp.cfg" \
  -c "init" \
  -c "halt" \
...
```sh
openocd -f "../../scripts/openocd/zynqmp/ftdi_zcu104.cfg" -f "target/xilinx_zynqmp.cfg" -c "reset_config srst_only"
```
...
```sh
openocd -f "../../scripts/openocd/zynqmp/ftdi_zcu104.cfg" -f "target/xilinx_zynqmp.cfg" -c "reset_config srst_only" -c "init" -c "core_up 1 2 3"
```
