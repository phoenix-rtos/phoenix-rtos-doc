...
power rails - see "Power and Status LEDs" section of the ZCU104 Board User Guide (UG1267) for detailed descriptions.
  ...
  - If the `DS35` LED is turned on (red), it indicates an error loading plo. Ensure that the boot mode is set correctly
  and the boot image is written correctly to the chosen boot medium (SD card or NOR flash).
...
plo on ZynqMP is configured with a RAM disk at address 0x08000000. The flash image can be written to it
