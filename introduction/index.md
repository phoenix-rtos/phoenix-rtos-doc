...
| 1996 | {violet-text}`Mac OS X`        | {violet-text}`Multitasking and multi-user operating system from Apple based on Mach microkernel and parts of BSD (XNU)` |
...
| 2008 | {violet-text}`iOS`             | {violet-text}`Mobile operating system for phones and tablets based on Mac OS X` |
